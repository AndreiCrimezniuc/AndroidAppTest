...
import { REWARDS_SERVICE_URL } from "../links";
...
export class RewardsService {
  ...
  async getDailyRewardStatus(maxRetries = 5): Promise<DailyRewardStatus> {
    if (!this.token) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Bad Request (attempt ${attempt}): ${this.url}/daily_reward/status`,
            {
        ...
        console.log(
          `✅ Good Request (attempt ${attempt}): ${this.url}/daily_reward/status`,
        );
        return data;
      } catch (err) {
        console.warn(
          `⚠️ Attempt ${attempt} to fetch daily reward status failed`,
          err,
        );
    ...
    throw new Error("Unexpected failure in getDailyRewardStatus()");
  }
...
export const useRewards = (token: string | null) => {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to fetch reward status";
