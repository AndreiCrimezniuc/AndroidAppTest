...
export const useEarnings = (token: string | null, isTokenReady: boolean) => {
        ...
        try {
            ...
            const errorMessage =
                err instanceof Error
                    ? err.message
                    : "Failed to fetch earnings: " + String(err) + "token: " + token;
    ...
    useFocusEffect(
        useCallback(() => {
            if (!token || !isTokenReady) return;

            fetchEarnings();
            const intervalId = setInterval(fetchEarnings, 2 * 60000);

            return () => clearInterval(intervalId);
        }, [token, isTokenReady, fetchEarnings])
    );
...
const Home: React.FC = () => {
    ...
    // Only fetch earnings when token is ready and initial loading is done
    const {
        earnings,
        isLoading: isLoadingEarnings,
        error: earningsError,
        refetch: refetchEarnings
    } = useEarnings(token, !isLoadingToken && !isInitialLoading);
